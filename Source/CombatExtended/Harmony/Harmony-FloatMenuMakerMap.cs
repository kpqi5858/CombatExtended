            ...
            if (compInventory != null)
            {
                ...
                if (!thingList.NullOrEmpty<Thing>())
                {
                    ...
                    if (item != null)
                    {
                        ...
                        {
                            opts.Add(new FloatMenuOption("CannotPickUp".Translate(item, item.Label) + " (" + "CE_InventoryFull".Translate() + ")", null));
                        }
                        // Pick up x
                        else if (count == 1)
                        {
                            opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("PickUp".Translate(item, item.Label), delegate
                            {
                        ...
                        else
                        {
                            ...
                            {
                                opts.Add(new FloatMenuOption("CannotPickUpAll".Translate(item, item.Label) + " (" + "CE_InventoryFull".Translate() + ")", null, MenuOptionPriority.Default, null, null, 0f, null, null));
                            }
                            else
                            {
                                opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("PickUpAll".Translate(item, item.Label), delegate
                                {
                                    item.SetForbidden(false, false);
                                    Job job = new Job(JobDefOf.TakeInventory, item);
                            ...
                            opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("PickUpSome".Translate(item, item.Label), delegate
                            {
                                int to = Mathf.Min(count, item.stackCount);
                                Dialog_Slider window = new Dialog_Slider("PickUpCount".Translate(item, item.LabelShort), 1, to, delegate (int selectCount)
                                {
                                    item.SetForbidden(false, false);
                                    Job job = new Job(JobDefOf.TakeInventory, item);
