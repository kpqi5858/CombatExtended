            ...
            if (compInventory != null)
            {
                ...
                if (!thingList.NullOrEmpty<Thing>())
                {
                    ...
                    if (item != null)
                    {
                        ...
                        {
                            opts.Add(new FloatMenuOption("CannotPickUp".Translate(item.Label, item) + " (" + "NoPath".Translate() + ")", null));
                        }
                        else if (!compInventory.CanFitInInventory(item, out count))
                        {
                            opts.Add(new FloatMenuOption("CannotPickUp".Translate(item.Label, item) + " (" + "CE_InventoryFull".Translate() + ")", null));
                        }
                        // Pick up x
                        else if (count == 1)
                        {
                        ...
                        else
                        {
                            ...
                            opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("PickUpSome".Translate(item.LabelNoCount, item), delegate
                            {
                                ...
                                Dialog_Slider window = new Dialog_Slider("PickUpCount".Translate(item.LabelNoCount, item), 1, to, delegate (int selectCount)
                                {
                                    item.SetForbidden(false, false);
                                    Job job = new Job(JobDefOf.TakeInventory, item);
